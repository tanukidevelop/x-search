...
`config.yaml` の `accounts` 配列にアカウントを追加します：
...
- 1行に1アカウント追加するだけで自動的に検索対象に含まれます
...
| 方式 | API コール数 | コスト（月額） |
|------|-----------|-----------|
| 各アカウント個別検索（非効率） | 2アカウント × 240回（日8回 × 30日） = **480回** | $2.40 |
| **OR クエリ統合検索（現在の方式）** | **1リクエスト × 240回 = 240回** | **$1.20** |

### クエリ長の制限

- X API v2 では最大 **500文字** のクエリが許可されています
- 10アカウント程度なら問題ありません
- 20個以上のアカウントを追加する場合は、複数グループに分けることをおすすめします
